...
from .gpdt_utils import flatten_prompt
import copy
from tqdm import tqdm


class GPDTSequenceTrainer:

    def __init__(self, model, optimizer, batch_size, get_batch, loss_fn,
                 scheduler=None, eval_fns=None, get_prompt=None, get_prompt_batch=None, get_prompt_batch_finetune=None, shift_action=False):
        self.model = model
        ...
        self.get_prompt_batch_finetune = get_prompt_batch_finetune
    ...
    def train_step_mix(self, no_prompt=False):
        # breakpoint()
        per_env_b_size, lengths, act_dims, prompt, batch = self.get_prompt_batch()
        ...
        if no_prompt:
            state_preds, action_preds, reward_preds = self.model.forward(
                per_env_b_size, lengths, states, actions, rewards, rtg[:,:-1], timesteps, attention_mask=attention_mask, prompt=None
            )
        else:
            state_preds, action_preds, reward_preds = self.model.forward(
                per_env_b_size, lengths, states, actions, rewards, rtg[:,:-1], timesteps, attention_mask=attention_mask, prompt=prompt
            )

        # breakpoint()
        loss = 0
        for idx, act_dim in enumerate(act_dims):
            start_idx = idx * per_env_b_size

            attention_mask_ = attention_mask[start_idx:start_idx+per_env_b_size]
            action_preds_ = action_preds[start_idx:start_idx+per_env_b_size, :, :act_dim].reshape(-1, act_dim)[attention_mask_.reshape(-1) > 0]
            action_target_ = action_target[start_idx:start_idx+per_env_b_size, :, :act_dim].reshape(-1, act_dim)[attention_mask_.reshape(-1) > 0]

            loss += self.loss_fn(
                None, action_preds_, None,
                None, action_target_, None,
            )
        ...
        with torch.no_grad():
            ...
            self.diagnostics['training/action_error'] = loss.clone().detach().cpu().item()
    ...
    def finetune_eval_iteration_multienv(self, get_prompt, get_batch, test_prompt_trajectories_list, test_trajectories_list, 
                                ...
                                finetune_opt=False):
        print('evaluate at tasks (finetune): ', env_name_list)
        logs = dict()
        ...
        finetune_losses = []
        for _ in tqdm(range(variant['finetune_steps'])):
        ...
        logs['time/finetuning'] = time.time() - finetune_start
        logs['finetuning/train_loss_mean'] = np.mean(finetune_losses)
        logs['finetuning/train_loss_std'] = np.std(finetune_losses)
    ...
    def train_step(self, batch_size_overwrite=None, optimizer=None):
        per_env_b_size, lengths, act_dims, prompt, batch = self.get_prompt_batch_finetune(batch_size=batch_size_overwrite)
        states, actions, rewards, dones, rtg, timesteps, attention_mask = batch
        action_target = torch.clone(actions)
        if self.shift_action:
            action_target = action_target[:, 1:, :]
        ...
        with torch.no_grad():
            self.diagnostics['finetuning/action_error'] = loss.clone().detach().cpu().item()
